...
import { PREFIX, StandupDoc, StandupStatus, ensureTeamForViewer, findBlob, getTeam, teamKey, usersKey } from './_lib/store.js'
...
const DebugQuery = z.object({ op: z.literal('debug-team') })
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // GET /api/kpi?op=debug-team
  if (DebugQuery.safeParse(req.query).success || op === 'debug-team') {
    ...
    return json(res, 200, {
      ...
      team: team || null,
      note: 'Debug payload (consolidated KPI endpoint).',
    })
  }

  // Default: GET /api/kpi?op=team
  if (op !== 'team' && op !== '') return json(res, 400, { error: 'Invalid op' })
  ...
  const teamCompliancePercent = users.length ? users.reduce((acc, u) => acc + u.weeklyAveragePercent, 0) / users.length : 0
